...
userRoutes.route("/buyer/cart").post(function(req,res){
    ...
    const {seller_name,buyer_name,cart} = req.body;
    let price =0;
...
for(var i=0 ; i<cart.length;i++){
    ...
    Products.find({$and:[{name:name},{ items_left: { $gte: Number(num) }},{status:"Available"},{seller_id:seller_name} ] },function(err,product){
           if (err){
            console.log(err)
        }
        else{
            if(product.length===0){
                send.stats=1;
                send.msg="Amount entered too large for " + name;
                res.json(send);
                return;
            }
            else{
                Products.update({name:name},{$inc:{items_left:-1*Number(num)}},function(err,car){
                        if (err){
                        console.log(err);
                        send.status=2;
                        ...
                        quantity_total=quantity_total + Number(num);
                        price = price + ((Number(car.price)*Number(num)));
                        if(i===cart.length){
                        ...
                        newbooking.save();
                        send.status=0;
                        send.msg="Order succesfully placed collect it in 1 hour!";
                        res.json(send) 
                    }
